...
In addition this RainRGB4RunCommand.exe version will write the selected color code (or the original color code if the dialog is canceled or exited) to STDOUT (Standard Out). This MUST be used by executing the RainRGB4RunCommand.exe program using the RunCommand plugin in Rainmeter, rather than just as a bang on an action. You can use the value of the RunCommand measure to !SetVariable or !SetOption or take other actions based on the color the user has chosen, without having to refresh the skin.
